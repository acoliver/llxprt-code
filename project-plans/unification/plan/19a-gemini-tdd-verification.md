...
# Phase 19a: Gemini Provider TDD Verification

## Phase ID

`PLAN-20250823-UNIFICATION.P19a`
...
- `packages/core/src/providers/gemini/verification/gemini-tdd-verification.md` - Verification document for TDD tests
  - MUST include: `@plan:PLAN-20250823-UNIFICATION.P19a`
...
- `packages/core/src/providers/gemini/index.ts`
  ...
  - ADD comment: `@plan:PLAN-20250823-UNIFICATION.P19a`
...
```markdown
<!--
 * @plan PLAN-20250823-UNIFICATION.P19a
 * @requirement REQ-XXX
...
grep -r "@plan:PLAN-20250823-UNIFICATION.P19a" packages/core/src/providers/gemini/ | wc -l
...
grep -r "toHaveBeenCalled\|toHaveBeenCalledWith" packages/core/src/providers/gemini/
# Expected: 0 occurrences

# Check for reverse testing
grep -r "toThrow('NotYetImplemented')\|expect.*not\.toThrow()" packages/core/src/providers/gemini/
# Expected: 0 occurrences

# Check for structure-only testing
grep -r "toHaveProperty\|toBeDefined\|toBeUndefined" packages/core/src/providers/gemini/ | \
  grep -v "with specific value"
# Expected: 0 occurrences

# Verify behavioral assertions exist
grep -E "toBe\(|toEqual\(|toMatch\(|toContain\(" packages/core/src/providers/gemini/ > /dev/null
# Expected: Tests have behavioral assertions

# Verify property-based tests included
grep -c "test\.prop\(" packages/core/src/providers/gemini/
# Expected: 30%+ of tests are property-based

# Run tests - should fail naturally with stub implementation
npm test packages/core/src/providers/gemini/ 2>&1 | head -20
...
- [ ] Phase 19 markers present (Gemini TDD tests completed)
...
- TDD tests for Gemini provider created and verified
...
1. Rollback commands: 
   ```bash
   rm -f packages/core/src/providers/gemini/verification/gemini-tdd-verification.md
   ```
2. Files to revert: Gemini provider TDD verification file
3. Cannot proceed to Phase 20 until fixed

## Phase Completion Marker

Create: `project-plans/unification/.completed/P19a.md`
Contents:
...
Files Created: 
- packages/core/src/providers/gemini/verification/gemini-tdd-verification.md
Files Modified: 
- packages/core/src/providers/gemini/index.ts
Tests Added: 0
