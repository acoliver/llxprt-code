...
const EXTENSIONS_DIRECTORY_NAME = path.join(LLXPRT_DIR, 'extensions');
...
describe('loadExtensions', () => {
  ...
  beforeEach(() => {
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'llxprt-code-test-workspace-'),
    );
...
>>>>>>> 2fc857092 (fix(trust): Refuse to load extensions from untrusted workspaces (#7342))
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'LLXPRT.md'),
    ]);
